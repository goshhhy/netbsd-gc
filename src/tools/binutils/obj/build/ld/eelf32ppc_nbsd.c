...
/* 32 bit ELF emulation code for elf32ppc_nbsd
...
#define TARGET_IS_elf32ppc_nbsd
...
static void gldelf32ppc_nbsd_before_parse (void);
static void gldelf32ppc_nbsd_after_parse (void);
static void gldelf32ppc_nbsd_after_open (void);
static void gldelf32ppc_nbsd_before_allocation (void);
static void gldelf32ppc_nbsd_after_allocation (void);
static lang_output_section_statement_type *gldelf32ppc_nbsd_place_orphan
  (asection *, const char *, int);

static void
gldelf32ppc_nbsd_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
ppc_before_allocation (void)
{
  ...
  gldelf32ppc_nbsd_before_allocation ();
...
static void
gldelf32ppc_nbsd_before_parse (void)
{
...
static void
gldelf32ppc_nbsd_after_parse (void)
{
...
static bfd_boolean
gldelf32ppc_nbsd_load_symbols (lang_input_statement_type *entry)
{
  int link_class = 0;
...
static void
gldelf32ppc_nbsd_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldelf32ppc_nbsd_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldelf32ppc_nbsd_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needs != NULL)
	{
	  ...
	  lang_for_each_input_file (gldelf32ppc_nbsd_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldelf32ppc_nbsd_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldelf32ppc_nbsd_search_needed (const char *path,
				    struct dt_needed *n, int force, int prepend_sysroot)
{
  ...
  if (name[0] == '/') {
    if (prepend_sysroot && ld_sysroot) {
      ...
      rv = gldelf32ppc_nbsd_try_needed (&needed, force);
      free(filename);
      return rv;
    }
    return gldelf32ppc_nbsd_try_needed (n, force);
  }
  ...
  while (1)
      ...
      else
	{
      ...
      needed.name = filename;
      if (gldelf32ppc_nbsd_try_needed (&needed, force))
	return TRUE;
...
static char *
gldelf32ppc_nbsd_add_sysroot (const char *path)
{
  int len, colons, i;
  char *ret, *p;
...
static void
gldelf32ppc_nbsd_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldelf32ppc_nbsd_after_open (void)
{
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldelf32ppc_nbsd_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldelf32ppc_nbsd_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldelf32ppc_nbsd_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  ...
	  if (gldelf32ppc_nbsd_search_needed (command_line.rpath_link,
						  &n, force, 0))
	    break;
	  if (gldelf32ppc_nbsd_search_needed (command_line.rpath,
						  &n, force, 1))
	    break;
	  found = 0;
	  ...
	  for (; !found && rp != NULL; rp = rp->next)
		...
		tmpname = gldelf32ppc_nbsd_add_sysroot (tmpname);
	      found = (rp->by == l->by
		       && gldelf32ppc_nbsd_search_needed (tmpname,
							      &n,
							      force, 1));
	      if (tmpname != rp->name)
		free ((char *) tmpname);
	  ...
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldelf32ppc_nbsd_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldelf32ppc_nbsd_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldelf32ppc_nbsd_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldelf32ppc_nbsd_find_exp_assignment (exp->binary.lhs);
      gldelf32ppc_nbsd_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldelf32ppc_nbsd_find_exp_assignment (exp->trinary.cond);
      gldelf32ppc_nbsd_find_exp_assignment (exp->trinary.lhs);
      gldelf32ppc_nbsd_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldelf32ppc_nbsd_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldelf32ppc_nbsd_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldelf32ppc_nbsd_find_exp_assignment (s->assignment_statement.exp);
}


/* used by before_allocation and handle_option. */
static void
gldelf32ppc_nbsd_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
...
static void
gldelf32ppc_nbsd_before_allocation (void)
{
  const char *rpath;
  asection *sinterp;
  bfd *abfd;
  ...
  if (is_elf_hash_table (link_info.hash))
    {
	 ...
	 referred to by dynamic objects.  */
      lang_for_each_statement (gldelf32ppc_nbsd_find_statement_assignment);
    }
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldelf32ppc_nbsd_append_to_separated_string (&depaudit, cp);
...
static bfd_boolean
gldelf32ppc_nbsd_open_dynamic_archive
...
static lang_output_section_statement_type *
gldelf32ppc_nbsd_place_orphan (asection *s,
				   const char *secname,
				   int constraint)
{
...
static void
gldelf32ppc_nbsd_after_allocation (void)
{
  ...
  if (need_layout < 0)
    ...
    gldelf32ppc_nbsd_map_segments (need_layout);
}

static char *
gldelf32ppc_nbsd_get_script (int *isfile)
{
  *isfile = 0;
...
static bfd_boolean
gldelf32ppc_nbsd_handle_option (int optc)
{
  switch (optc)
    {
    default:
	...
	einfo (_("%P%F: invalid --compress-debug-sections option: `%s'\n"),
      ...
      break;
    case OPTION_AUDIT:
	gldelf32ppc_nbsd_append_to_separated_string (&audit, optarg);
	break;

    case 'P':
	gldelf32ppc_nbsd_append_to_separated_string (&depaudit, optarg);
	break;
...
static void
gldelf32ppc_nbsd_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_elf32ppc_nbsd_emulation =
{
  gldelf32ppc_nbsd_before_parse,
  syslib_default,
  hll_default,
  gldelf32ppc_nbsd_after_parse,
  ppc_after_open,
  gldelf32ppc_nbsd_after_allocation,
  set_output_arch_default,
  ldemul_default_target,
  ppc_before_allocation,
  gldelf32ppc_nbsd_get_script,
  "elf32ppc_nbsd",
  "elf32-powerpc",
  ppc_finish,
  ppc_after_open_output,
  gldelf32ppc_nbsd_open_dynamic_archive,
  gldelf32ppc_nbsd_place_orphan,
  NULL,
  NULL,
  gldelf32ppc_nbsd_add_options,
  gldelf32ppc_nbsd_handle_option,
  NULL,
  gldelf32ppc_nbsd_list_options,
  gldelf32ppc_nbsd_load_symbols,
  NULL,
  NULL,
  NULL
};
